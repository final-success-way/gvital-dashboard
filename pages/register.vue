<template>
  ...
  >
    <h3 class="font-bold text-3xl mb-4">Register</h3>
    ...
    <!--    <div class="w-full">-->
    <!--      <label class="font-medium text-xs">Instagram Username <span class="ml-1 text-star">*</span></label>-->
    <!--      <div class="my-2 w-full flex items-center">-->
    <!--        <input type="text"-->
    <!--               v-model="user.username"-->
    <!--               required-->
    <!--               @input="clearWarning"-->
    <!--               :class="error=='username'?'bg-youtube_bg':''"-->
    ...
    <!--      </div>-->
    <!--      <p v-if="accountPrivate" class="text-xs font-medium text-youtube">This Instagram username is set to private.-->
    <!--        Please set it to Public and try again.</p>-->
    <!--    </div>-->

    <div class="w-full">
      <label class="font-medium text-xs"
        >User Email <span class="ml-1 text-star">*</span></label
      >
      <div class="my-2 w-full flex items-center">
        <input
          type="email"
          ...
          @input="clearWarning"
          class="w-full border border-light_thin px-4 py-3 rounded-lg focus:border-primary focus:outline-none"
          :class="error == 'email' ? 'bg-youtube_bg' : ''"
    ...
    <p class="m-0 text-xs text-center my-2">
      ...
      <a href="/terms" class="underline">terms and conditions</a>
    </p>
  </form>
</template>
...
export default {
  ...
  data: () => ({
    ...
    errorMessage: false,
    create_account: true,
    message: 'Get your 100 free followers now',
    btnMessage: 'Get free followers',
  ...
  head() {
    return {
      title:
        'Register | Buy Instagram Followers, Twitter Followers and YouTube Subscribers - Buy Real Follows',
      meta: [
        { charset: 'utf-8' },
      ...
      link: [
        {
          ...
          href: this.host.includes('www')
            ? `https://${this.host}/register`
            : `https://www.${this.host}/register`,
        },
  ...
  components: { LoaderSmall, InstaInfo },
  methods: {
    clearWarning() {
    ...
    async registerUser() {
      ...
      try {
        const resp = await this.$axios.post('register/', {
          insta_username: this.user['username'],
