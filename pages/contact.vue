<template>
  <form
    ...
    class="container mx-auto mt-16 mb-40 w-4/5 lg:w-2/5"
  >
    <h3 class="font-bold text-3xl mb-4">Contact us</h3>
    <transition name="fade">
      ...
      <p
        v-if="message"
        class="text-xs p-4 rounded-lg font-medium text-center my-4"
      >
        {{ message }}.
      </p>
    </transition>
    ...
    <div class="w-full">
      <label class="font-medium text-xs"
        >Name <span class="ml-1 text-star">*</span></label
      >
      <div class="my-2 w-full flex items-center">
        <input
          type="text"
    ...
    <div class="w-full">
      <label class="font-medium text-xs"
        >Message <span class="ml-1 text-star">*</span></label
      >
      <div class="my-2 w-full flex items-center">
        <textarea
          type="text"
          ...
          @input="clearWarning"
          class="w-full h-48 border border-light_thin px-4 py-3 rounded-lg focus:border-primary focus:outline-none"
  ...
  <div
    v-else-if="requestSent === true && !error"
  ...
  >
    ...
    <h3 class="font-bold text-3xl text-center">Your request received</h3>
    <p class="m-0 text-xs text-center my-2">
      We have received your request, we will get back to you shortly on the
      email you've shared
    </p>
  </div>
  <div
  ...
  >
    ...
    <h3 class="font-bold text-3xl text-center">Something error occurred</h3>
    <p class="m-0 text-xs text-center my-2">
...
export default {
  ...
  head() {
    return {
      title:
        'Contact | Buy Instagram Followers, Twitter Followers and YouTube Subscribers - Buy Real Follows',
      meta: [
        { charset: 'utf-8' },
        { name: 'viewport', content: 'width=device-width, initial-scale=1' },
        {
          ...
          name: 'description',
          content:
            'Buy real follows provide cheapest instagram followers, instagram likes, youtube likes, youtube subscribers, facebook likes and more. Check our cheapest service with the best rates in the market. Services start instantly and 24 Hrs customer supports.',
        },
        {
          hid: 'keywords',
          name: 'keywords',
  ...
  methods: {
    ...
    async submitContact(e) {
      e.preventDefault()
      this.loading = true
      const { payment_session_id } = this.$route.query
      try {
        this.user['extra'] = payment_session_id
        const resp = await this.$axios.post('contact-request/', this.user)
